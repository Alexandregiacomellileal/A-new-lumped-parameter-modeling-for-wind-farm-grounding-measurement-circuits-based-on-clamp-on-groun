...
In Figure 2, $R_f$ corresponds to the turbine grounding resistance (Ω), $C_f$ corresponds to the turbine grounding capacitance (F), $L_p$ is the horizontal electrodes' self-inductance (H), $R_s$ is the horizontal electrodes' self-resistance (Ω), $R_p$ is the horizontal electrodes grounding resistance (Ω), and $C_p$ corresponds to the horizontal electrode capacitance (F). Furthermore, in a new proposed modeling, a resistance $R_c$ (Ω), a capacitance $C_c$ (F) and an adjustment parameter $k$ were added to the lumped-parameter model to represent the impact of mutual resistance between the grounding elements and mitigate distortions due to modeling. To represent the mutual couplings between the grounding of the turbine under test and its adjacent horizontal electrodes, the resistance $R_c$ ($\Omega$), and the capacitance $C_c$ ($F$) are calculated as $R_c = (R_f + R_p)  \frac{k}{1-k}$, and $C_c = \frac{C_f \ C_p} {C_f + C_p} \frac{1-k}{k}$.
