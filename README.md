...
to represent the mutual couplings between the grounding of the turbine under test and its adjacent horizontal electrodes, the resistance $R_c$ ($\Omega$), and the capacitance $C_c$ ($F$) are calculated as $R_c = (R_f + R_p)  \frac{k}{1-k}$, and $C_c = \frac{C_f \ C_p} {C_f + C_p} \frac{1-k}{k}$.
