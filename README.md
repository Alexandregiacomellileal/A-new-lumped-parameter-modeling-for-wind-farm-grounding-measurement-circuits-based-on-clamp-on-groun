...
This section presents the methodology used to tune the parameter $k$ in the lumped-parameter model. The parameter $k$ is calculated as $k= (1- e_{total})$, and $e_{total}$ is the sum of the measurement error caused by the mutual coupling impedance between the turbine grounding and the horizontal electrode distanced by $s$ meters, plus the error caused by the representation of the horizontal electrode by a pair of hemispherical electrodes (lumped parameter modeling), being calculated as: $e_{total}=\frac{\left({{Zmed}_{wire}^\infty - Zmed}_{lumped}^{short}\right)}{{Zmed}_{wire}^\infty}$.
